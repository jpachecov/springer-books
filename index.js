...
const processFile = async (url, title, type) => {
  ...
  axios({
    ...
    response.data.pipe(fs.createWriteStream(`./books/${title}.${type}`))
  }).catch(e => {
    console.log(`Error processing or writing: http://link.springer.com/${url}`);
  })
}

const getValues = (html) => {
  const values = {};

  const titleSelector = '.page-title h1';
  const titleElement = $(titleSelector, html);
  const pdfSelector = '.cta-button-container__item a[title="Download this book in PDF format"]';
  const epubSelector = '.cta-button-container__item a[title="Download this book in EPUB format"]';
  const pdfElement = $(pdfSelector, html);
  const epubElement = $(epubSelector, html);

  values.title = titleElement.text();
  values.pdf = (pdfElement.length ? pdfElement.attr('href') : null);
  values.epub = (epubElement.length ? epubElement.attr('href') : null);

  return values;
}

const processList = async (urls) => {
  for (const url of urls) {
    await rp(url)
      .then(async (html) => {
        const {title, pdf, epub} = getValues(html);
        if(pdf) {
          await processFile(pdf, title, 'pdf');
        }
        if(epub) {
          await processFile(epub, title, 'epub');
        }
      })
      .catch((error) => console.log(`Error downloading: ${url}. ${error}`));
  }
}

processList(urlsList);
